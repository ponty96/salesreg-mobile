	...
	switch (type) {
		...
		case 'UpdateOrderStatus':
			return {
				...
				subtitle: `${params.contactName.split(' ')[0].trim()}'s order status has been updated`
			}
		...
		case 'DeleteAccount':
			return {
				...
				subtitle: `Account with number ${params.accountNumber} has been unlinked from account`
			}
		case 'CreateExpense':
			return {
		...
		case 'DeleteLegalDocument':
			return {
				...
				subtitle: `Your ${params.name} ${params.type == 'policy' ? 'policy' : ''} has been removed`
			}
	}
}
