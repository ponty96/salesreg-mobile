...
class UpsertInvoiceScreen extends React.PureComponent<IProps, IState> {
	...
	navigateUser = () => {
		const { navigation: { state: { params: { sales, from } } } } = this.props
		...
		let banner = NotificationBanner(configureNotificationBanner('MakeInvoicePayment', this.state.amountPaid))
	...
	onCompleted = async res => {
		const { createReceipt: { success, fieldErrors } } = res
		if (!success) {
	...
	checkInvoiceValidity = (makePayment: (obj: any) => void) => {
		let amountPayable = this.props.navigation.getParam('amountPayable', null),
			...
			{ navigation: { state: { params: { sales: { invoice: { id } } } } } } = this.props
...
const _UpsertInvoiceScreen: any = props => (
	<UserContext.Consumer>{({ user }) => <UpsertInvoiceScreen {...props} user={user} />}</UserContext.Consumer>
)
