...
import { View, StyleSheet, Text, Share, TouchableOpacity, TouchableWithoutFeedback, Linking } from 'react-native'
		...
		return {
			header: (
				<Header
					...
					onPressRightIcon={() => navigation.navigate('UpdateProduct', { product })}
				/>
			)
		}
	}
	...
	sections = (): any => {
		...
		return [
			...
			},
			{
				...
				hideBody: product.categories && product.categories.length > 0 ? false : true
			},
			// open tags screen, which shows list of products and services attached to the tag
			{
				...
				body: (
					<View style={styles.tags}>
						{product.tags && product.tags.map((tag, i) => <Chip key={i} text={tag.name} />)}
					</View>
				),
			...
			},
			{
				...
				hideBody: product.description && product.description.length > 0 ? false : true
			}
		]
	}
	...
	render() {
		...
		return [
			<View style={styles.topHeader} key="product-details-334">
				<TouchableOpacity onPress={this.onShare} style={{ flexDirection: 'row', alignItems: 'center' }}>
				...
				>
					...
					<Icon name="chevron-small-right" type="Entypo" style={styles.headerIcon} />
				</TouchableOpacity>
			</View>,
			<GenericProfileDetails
				...
				onSuccessfulDeletion={() => {
					let banner = NotificationBanner(configureNotificationBanner('DeleteProduct', product))
...
const _ProductDetailsScreen: any = props => (
	<UserContext.Consumer>{({ user }) => <ProductDetailsScreen {...props} user={user} />}</UserContext.Consumer>
)
