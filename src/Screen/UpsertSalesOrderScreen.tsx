...
class UpsertSalesOrderScreen extends React.PureComponent<IProps, IState> {
	...
	async componentDidMount() {
		const { company: { id: companyId }, id: userId } = JSON.parse(await Auth.getCurrentUser())
		this.setState({
			user: {
				companyId,
				userId
	...
	updateState = (key: string, val: any) => {
		...
		this.setState({
			...formData,
			amountPaid: key == 'items' ? total.toString() : key == 'amountPaid' ? val : this.state.amountPaid
		})
	}
	...
	navigateUser = () => {
		...
		let banner = NotificationBanner(configureNotificationBanner('UpsertSalesOrder'))
	...
	onCompleted = async res => {
		const { upsertSaleOrder: { success, fieldErrors, data } } = res
	...
	render() {
		const { isCustomerInContacts, user: { companyId } } = this.state
																...
																emptyText: (
																	<Text>
																		...
																		<Text style={{ color: color.button }}>
																			go back > Select No for the ("Is this customer
																			in your contacts?") question > Fill the
																			appropriate fields
																		</Text>
																	</Text>
																)
...
const _UpsertSalesOrderScreen: any = props => (
	<UserContext.Consumer>{({ user }) => <UpsertSalesOrderScreen {...props} user={user} />}</UserContext.Consumer>
)

_UpsertSalesOrderScreen.navigationOptions = UpsertSalesOrderScreen.navigationOptions
