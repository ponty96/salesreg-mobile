...
class DeliveryFeeScreen extends React.Component<IProps, IState> {
	...
	onCompleted = async res => {
		const { deleteDeliveryFee: { success, fieldErrors } } = res

		if (!success) {
			setTimeout(
				() =>
					Alert.alert('Error', fieldErrors[0].message, [{ text: 'Ok', onPress: () => null }], {
						cancelable: false
					}),
				100
			)
			...
			let banner = NotificationBanner(
				configureNotificationBanner('DeleteDeliveryFee', this.state.deliveryAddressToDelete)
			)
						...
						return (
							<React.Fragment>
								...
								<GenericListIndex
									...
									parseItemData={item => this.parseData(item, deleteDeliveryFee)}
									onItemPress={() => null}
...
const _DeliveryFeeScreen: any = props => (
	<UserContext.Consumer>{({ user }) => <DeliveryFeeScreen {...props} user={user} />}</UserContext.Consumer>
)
