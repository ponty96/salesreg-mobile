...
export default class SalesAnalytics extends React.PureComponent<IProps, IState> {
	constructor(props) {
		super(props)
		this.state = {
	...
	renderDueInvoice = data => {
		...
		return (
			<View style={styles.dueInvoiceContainer}>
				...
				<DemiBoldText style={styles.largeText}>{`\u20A6${numberWithCommas(amountDue || 0)}`} </DemiBoldText>
			</View>
		)
	}
	...
	renderSales = data => {
		...
		return (
			<React.Fragment>
				<DemiBoldText style={styles.largeText}>{`\u20A6${numberWithCommas(totalIncome)}`}</DemiBoldText>
				<View style={{ marginTop: 20 }}>
					<MediumText style={styles.smallText}>TOTAL PRODUCT</MediumText>
					<DemiBoldText style={styles.largeText}>{totalProducts || 0}</DemiBoldText>
				</View>
			</React.Fragment>
		)
	}
			...
			<View style={styles.productContainer}>
				...
				{topProducts.length > 0 ? (
					...
					<View style={styles.emptyContainer}>
						<RegularText style={[styles.smallText, styles.noDataText]}>No sales yet</RegularText>
					</View>
				)}
			</View>
		)
